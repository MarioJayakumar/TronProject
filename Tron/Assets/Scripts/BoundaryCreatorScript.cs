...
public class BoundaryCreatorScript : MonoBehaviour {
	...
	void Start () {
		...
		generalBoundary.transform.localScale = transform.localScale;

		topBoundary = (Rigidbody)Instantiate (generalBoundary, 
			new Vector3(transform.position.x +  xLength/2, transform.position.y, transform.position.z), 
			new Quaternion());
		...
		rightBoundary = (Rigidbody)Instantiate (generalBoundary, 
			new Vector3(transform.position.x, transform.position.y, transform.position.z - zLength/2), 
			new Quaternion());
